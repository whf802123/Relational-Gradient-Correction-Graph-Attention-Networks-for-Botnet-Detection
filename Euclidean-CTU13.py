...
def _safe_row_euclidean_distance(x_np: np.ndarray) -> np.ndarray:
    if x_np.shape[0] == 0:
    ...
    sq_norms = np.sum(x_np * x_np, axis=1, keepdims=True)
    dist_sq = sq_norms + sq_norms.T - 2.0 * (x_np @ x_np.T)
...
def build_train_graph(x_train_np: np.ndarray):
    ...
    distance = _safe_row_euclidean_distance(x_train_np)
    adj = (distance <= EUCLIDEAN_THRESHOLD)
...
def build_inductive_eval_graph(x_train_np: np.ndarray, x_test_np: np.ndarray):
    ...
    distance = _safe_row_euclidean_distance(x_eval_np)

    src_list = []
    dst_list = []

    if n_train > 0:
        distance_tt = distance[:n_train, :n_train]
        adj_tt = (distance_tt <= EUCLIDEAN_THRESHOLD)
        ...
        distance_train_test = distance[:n_train, n_train:]
        train_src, test_col = np.where(
            distance_train_test <= EUCLIDEAN_THRESHOLD
        )
        if train_src.size > 0:
        ...
        if NUM_CLASSES > 2:
            if y_prob_test_all.shape[0] == 0:
                ...
                if plotted:
                    ...
                    print(
                        "[ROC] No class has both positive and negative test samples. Multiclass ROC cannot be plotted.")
        else:
            print("[ROC] Binary classification detected. Skipping multiclass ROC.")
    except Exception as e:
