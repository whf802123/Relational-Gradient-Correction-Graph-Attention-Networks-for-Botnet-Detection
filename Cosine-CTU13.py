...
def _safe_row_cosine_similarity(x_np: np.ndarray) -> np.ndarray:
    if x_np.shape[0] == 0:
    ...
    cosine = np.divide(
        x_np @ x_np.T,
        denom,
        out=np.zeros((x_np.shape[0], x_np.shape[0]), dtype=np.float64),
        where=denom > 0.0,
    )
...
def build_train_graph(x_train_np: np.ndarray):
    ...
    cosine = _safe_row_cosine_similarity(x_train_np)
    adj = (np.abs(cosine) >= COSINE_THRESHOLD)
...
def build_inductive_eval_graph(x_train_np: np.ndarray, x_test_np: np.ndarray):
    ...
    cosine = _safe_row_cosine_similarity(x_eval_np)

    src_list = []
    dst_list = []

    if n_train > 0:
        cosine_tt = cosine[:n_train, :n_train]
        adj_tt = (np.abs(cosine_tt) >= COSINE_THRESHOLD)
        ...
        cosine_train_test = cosine[:n_train, n_train:]
        train_src, test_col = np.where(np.abs(cosine_train_test) >= COSINE_THRESHOLD)
        if train_src.size > 0:
