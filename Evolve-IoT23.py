...
CSV_PATH = 'C:\\Users\\whf80\\Desktop\\DW-GAT\\ICASSP\\iot23_combined_new.csv'
WINDOW_SIZE = 1000
BATCH_SIZE = 100
...
if WINDOW_SIZE % BATCH_SIZE != 0:
    raise ValueError('WINDOW_SIZE must be divisible by BATCH_SIZE.')
...
rare_labels_to_drop = {'C&C-FileDownload', 'C&C-Torii', 'FileDownload', 'C&C-HeartBeat-FileDownload', 'Okiru-Attack', 'C&C-Mirai', 'Attack'}
before = len(df)
...
df = df.groupby('label', group_keys=False).apply(keep_or_sample).reset_index(drop=True)
...
num_cols_raw = [c for c in ['ts', 'id.orig_p', 'id.resp_p', 'duration', 'orig_bytes', 'resp_bytes', 'missed_bytes', 'orig_pkts', 'orig_ip_bytes', 'resp_pkts', 'resp_ip_bytes'] if c in df.columns]
cat_cols = [c for c in ['proto', 'service', 'conn_state', 'history', 'local_orig', 'local_resp'] if c in df.columns]
for c in ip_cols:
...
if 'ts' in df.columns and df['ts'].notna().any():
    df = df.sort_values(['ts', 'num'], kind='mergesort', na_position='last').reset_index(drop=True)
else:
    df = df.sort_values('num', kind='mergesort').reset_index(drop=True)
...
if N_total < WINDOW_SIZE:
    raise ValueError('Sampled dataset is smaller than WINDOW_SIZE; reduce WINDOW_SIZE and BATCH_SIZE.')
...
if len(y_true_test) == 0:
    ...
    try:
        ...
        if len(y_true_test) == 0:
            ...
            print(f'[ROC] Sample count mismatch: y_true={len(y_true_test)}, y_prob={y_prob_test_all.shape[0]}')
        elif y_prob_test_all.shape[1] != NUM_CLASSES:
            print(f'[ROC] Probability matrix class dimension mismatch: expected={NUM_CLASSES}, actual={y_prob_test_all.shape[1]}')
        else:
            ...
            if NUM_CLASSES > 2:
                ...
                for cls_id in classes_sorted:
                    ...
                    print(f'[ROC] {class_name}: positive={n_pos}, negative={n_neg}')
                    if n_pos == 0 or n_neg == 0:
                        print(f'[ROC] Skip {class_name}: missing positive or negative samples.')
                        continue
                    (fpr, tpr, _) = roc_curve(y_true_c, y_score_c)
                    roc_auc = auc(fpr, tpr)
                    ax_roc.plot(fpr, tpr, linewidth=1.5, label=f'{class_name} (AUC={roc_auc:.3f})')
                    plotted = True
            elif NUM_CLASSES == 2:
                y_true_c = y_true_test
                y_score_c = y_prob_test_all[:, 1]
                if len(np.unique(y_true_c)) < 2:
                    print('[ROC] The binary test set contains only one class, so ROC cannot be computed.')
                else:
                    (fpr, tpr, _) = roc_curve(y_true_c, y_score_c)
                    roc_auc = auc(fpr, tpr)
                    ax_roc.plot(fpr, tpr, linewidth=1.8, label=f'ROC (AUC={roc_auc:.3f})')
                    plotted = True
            if plotted:
                ax_roc.plot([0, 1], [0, 1], linestyle='--', linewidth=1.0, label='Random')
